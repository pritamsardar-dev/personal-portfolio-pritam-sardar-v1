import React from "react";

import clsx from "clsx";

import { currentSkillsSnapshotOverviewBlockLayoutConfig } from "./currentSkillsSnapshotOverviewBlockLayout.config";

import Text from "../../../atoms/text/Text";

const {
  blockInnerContainer: blockInnerContainerClasses,
  bodyItemsContainer: bodyItemsContainerClasses,
  bodyItemContainer: bodyItemContainerClasses,
  alignmentMap,
} = currentSkillsSnapshotOverviewBlockLayoutConfig;

// CMS driven Current Skills Snapshot overview block.
// Renders an optional heading and ordered body items with token driven alignment.
const CurrentSkillsSnapshotOverviewBlock = ({ data = {}, className, ...props }) => {
  const {
    id,
    enabled = true,
    heading,
    bodyItems = [],
    alignment = {
      heading: "left",
      body: "left",
    },
  } = data;

  if (!enabled) return null;

  return (
    <div
      id={id}
      className={clsx(
        blockInnerContainerClasses,
        alignmentMap[alignment.heading] || alignmentMap.left,
        className,
      )}
      {...props}
    >
      {/* Block Heading */}
      {heading && <Text {...heading} />}

      {/* Body Items */}
      {Array.isArray(bodyItems) && bodyItems.length > 0 && (
        <div
          className={clsx(
            bodyItemsContainerClasses,
            alignmentMap[alignment.body] || alignmentMap.left,
          )}
        >
          {bodyItems.map((item) => (
            <div key={item.id} className={bodyItemContainerClasses}>
              {item.heading && <Text {...item.heading} />}
              {item.body && <Text {...item.body} />}
            </div>
          ))}
        </div>
      )}
    </div>
  );
};

export default CurrentSkillsSnapshotOverviewBlock;
