...
const ListContentBlock = ({ items = {}, labelValueItems = {}, inlineItems = {}, className }) => {
  const hasListItems = Array.isArray(items?.texts) && items.texts.length > 0;

  const hasLabelValueItems =
    Array.isArray(labelValueItems?.texts) && labelValueItems.texts.length > 0;

  const hasInlineItems = Array.isArray(inlineItems?.texts) && inlineItems.texts.length > 0;
        ...
        <ul className={listClasses}>
          {labelValueItems.texts.map((pair, idx) => (
            <li key={idx} className={labelValueRowClasses}>
              ...
              {pair.value && <Text variant={labelValueItems.variant} text={pair.value} />}
            </li>
          ))}
        </ul>
      )}
      ...
      {hasListItems && (
        <ul className={listClasses}>
          {items.texts.map((text, idx) => (
            <Text key={idx} variant={items.variant} as={items.as} text={text} />
          ))}
        </ul>
      )}
        ...
        <div>
          {inlineItems.texts.map((text, idx) => (
            <React.Fragment key={idx}>
              <Text variant={inlineItems.variant} text={text} className="inline" />
              {idx < inlineItems.texts.length - 1 && (
                <Text text={inlineItems.separator ?? " · "} className="inline" />
              )}
            </React.Fragment>
          ))}
        </div>
